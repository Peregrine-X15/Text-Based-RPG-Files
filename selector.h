...
struct SelectorParams
{
	...
	SelectorParams(
		//sizes may vary, indicates selector position in matrix
		int x = 20, int y = 20, int columns = 1, int currentX = 0, int currentY = 0,
...
inline void Selector<T>::ProcessInputs()
{
	if (m_Keyboard.IsKeyJustPressed(KEY_W))
		...
		MoveRight();
	else if (m_Keyboard.IsKeyJustPressed(KEY_E))
		OnAction();
}
...
inline void Selector<T>::DrawItem(int x, int y, T item)
{
	...
	TRPG_ERROR("Data type [" + type + "]-needs to be a wstring, please create the draw item function for override");
}
